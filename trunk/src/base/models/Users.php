...
class Users extends Model {
    ...
    function __construct() {
        $this->bcrypt = new Bcrypt(self::BCRYPT_ROUNDS);
    }
    ...
    public function login($username,$password,$sticky=false) {
        $sql = 'SELECT user_id,password FROM '.$this->table.' WHERE
                username = ? AND is_active = 1;';

        $results = $this->getOne($sql,array($username));
        if (empty($results)) return false;
        else {
            $passChk = $this->bcrypt->verify(md5($password),$results[1]);
            if (!empty($passChk)) {
                $user = new Users();
                $user->load($results[0]);
