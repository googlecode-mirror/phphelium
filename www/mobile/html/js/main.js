...
var htmlAssist = {
    hideOverlay: function(ident) {
        $('#'+ident+'Overlay').hide();
    },
    
    showOverlay: function(ident,contain) {
        if (document.getElementById(ident+'Overlay')) {
            htmlAssist.correctOverlay(ident,contain);
            $('#'+ident+'Overlay').show();
        } else {
            ...
            li.src = "/images/loading.gif";
            li.id = ident+'OImg';

            ol.appendChild(li);
            mo.appendChild(ol);

            document.getElementById(contain).insertBefore(mo,document.getElementById(contain).firstChild);
            
            htmlAssist.correctOverlay(ident,contain);
        }
    },

    correctOverlay: function(ident,contain) {
        $('#'+ident+'Overlay').css('height',(parseInt($('#'+contain).css('height').replace('px',''))+20)+'px');
        $('#'+ident+'Overlay').css('width',(parseInt($('#'+contain).css('width').replace('px',''))+20)+'px');

        $('#'+ident+'Overlay').css('marginTop','-10px');
        $('#'+ident+'Overlay').css('marginLeft','-10px');

        $('#'+ident+'OLoad').css('marginTop',((parseInt($('#'+contain).css('height').replace('px',''))/2)-parseInt($('#'+ident+'OLoad').css('height').replace('px','')))+'px');
    },

    noEmpties: function(formId) {
        var empties = false;
        $('form#'+formId+' :input').each(function(i) {
    ...
    carriageTriggers: function(formId,followOn) {
        $('form#'+formId+' :input').each(function(i) {
            if (this.type == 'text' || this.type == 'password' || this.type == 'textarea') {
                $(this).keyup(function(e) {
                    if (e.keyCode == 13 && htmlAssist.noEmpties(formId) == false) {
                        followOn(true);
                    }
                });
            }
